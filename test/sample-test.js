...
describe("NFTMarket", function () {
    it("Should create and  execute market sales", async function () {
        ...
        await market.createMarketItem(
            nftContractAddress, 1, auctionPrice,
            {value: listingPrice})
        await market.createMarketItem(
            nftContractAddress, 2, auctionPrice,
            {value: listingPrice})

        const [_,
            buyerAddress] = await ethers.getSigners()

        await market.connect(buyerAddress).connect(buyerAddress).createMarketSale(
